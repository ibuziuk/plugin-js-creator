...
public class DomUtil {
	...
	private static boolean isSuitableJsModule(Node node) {
		Node parentNode = node.getParentNode();
		if (parentNode.getNodeType() == Node.ELEMENT_NODE) {
			Element parentElement = (Element) parentNode;
			if (parentElement.getNodeName().equals(PLUGIN)) { // Common js-module for all types of projects (ios, android, wp8 etc.)
				return true;
			} else if (parentElement.getNodeName().equals(PLATFORM)) {
				String platformName = parentElement.getAttribute(NAME_ATTRIBUTE);
				if (platformName.equals(ANDROID)) { // CordovaSim supports only android and hybrid projects
					return true;
				}
			}
		}
